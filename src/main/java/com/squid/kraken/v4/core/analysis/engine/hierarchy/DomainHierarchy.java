...
public class DomainHierarchy {
	...
	/**
	 * allow to add more hierarchy - this is used for lazy sub-domains
	 * intialization
	 * 
	 * @param hierarchy
	 */
	...
	/**
	 * return any Dimension, Metric or Relation that can be referenced by the
	 * name - or null if nothing found
	 * 
	 * @param name
	 */
	...
	/**
	 * get the dimension identified by its PK - or throw an Exception if cannot
	 * find or cannot access
	 * 
	 * @param ctx
	 * @param dimensionPK
	 * @return
	...
	public boolean isDone(Integer timeoutMs) throws InterruptedException, TimeoutException, ExecutionException {
		...
		if ((this.state == State.CANCELLED) || (this.state == State.DONE)) {
			...
			if (compute!=null && this.state == State.STARTED) {
	...
	/**
	 * cancel the domain computation if needed - wait at least 10s to make sure
	...
	private void loadHierarchies(Space root, List<List<DimensionIndex>> structure) {
		...
		// flatten
		HashSet<DomainPK> domains = new HashSet<DomainPK>();// list the
															// sub-domains
		...
		for (DimensionIndex i : flatten) {
			if (i.getAxis().getDimension().getImageDomain().isInstanceOf(IDomain.CONDITIONAL) && (i.getParent() == null)
	 ...
	 * 
	 * @param axis
	 * @return the index or NULL if not defined (this may happens if the model
