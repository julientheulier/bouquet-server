...
public class Axis implements Property {
	...
	public DimensionIndex getIndex(boolean wait) throws ComputingException, InterruptedException {
		...
		DomainHierarchy hierarchy = universe.getDomainHierarchy(parent.getRoot(), false);// the index is attached to the root; in case of a sub-dimension, the index is a proxy to the actual dimension. If you think it should be parent.getDomain(), that's not good...
		if (hierarchy!=null) {
