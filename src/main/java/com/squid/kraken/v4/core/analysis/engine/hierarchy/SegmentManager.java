...
/**
 * handling segment facet - that is a facet build using all the conditional
 * dimensions from the domain
 * 
 * @author sergefantino
 *
 */
public class SegmentManager {
	...
	public static Facet createSegmentFacet(Universe universe, DomainHierarchy hierarchy, Domain domain, String facetId,
		...
		boolean allow_open_filters = true;// universe.hasRole(domain,
											// Role.WRITE);
		// first check the filter. If it's an expression, use it also to filter
		// existing facets (KRKN-47)
		if (filter != null && !filter.isEmpty() && filter.startsWith("=")) // formula
																			// starts
																			// with
																			// =,
																			// like
	...
	public static void addSegmentSelection(AppContext ctx, Universe universe, Facet facet, DashboardSelection ds)
			throws ComputingException, InterruptedException, ScopeException {
		DomainPK pk = new DomainPK(facet.getDimensionId().getCustomerId(), facet.getDimensionId().getProjectId(),
				facet.getDimensionId().getDomainId());
		...
		for (FacetMember selectedItem : facet.getSelectedItems()) {
			if (selectedItem instanceof FacetMemberString) {
				...
				if (isOpenFilter(fmember)) {
					...
					if (formula.startsWith("=")) {// if start with equal, remove
													// it
	...
	/**
	 * support for open filters (KRKN-47). Check if the facetMember is actually
	 * defining an open filter
	 * 
	 * @param id
	 * @return
	 */
