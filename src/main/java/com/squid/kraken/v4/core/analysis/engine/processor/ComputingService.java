...
/**
 * This is the main entry point for compute API. It supports performing
 * execution of a analysisJob and a facetJob. Also this service is responsible
 * for initialization of domain Hierarchy: this is not exposed by API directly
 * but called as a side-effect when needed.
 * 
 * The service is re-entrant, that is it is possible to call the
 * ComputingService more than one time in the same callstack.
 * 
...
public class ComputingService {
	...
	/**
	 * execute the DashboardAnalysis. Delegate actual execution to an
	 * AnalysisCompute class. Return a DataMatrix. In order to do so the whole
	 * resultset will be loaded in memory. Once the DataMatrix is released, in
	 * memory resultset is managed by the RowCache.
	 * 
	 * @param analysis
	 * @param redis
	 *            key if available
	...
	public DataMatrix glitterAnalysis(final DashboardAnalysis analysis, String key)
	...
	public DataMatrix glitterAnalysis(final DashboardAnalysis analysis)
			throws ComputingException, InterruptedException {
		return this.glitterAnalysis(analysis, null);
	}

	/**
	 * execute the DashboardAnalysis but does not actually read the resultset:
	 * calling method is responsible for reading and managing in memory
	 * representation. It can be used to stream the result for instance without
	 * reading it in-memory first. That call will not cache the resultset as a
	 * RowCache.
	 * 
	 * @param analysis
	 * @return
	 ...
	 */
	/*
	 * public ExecuteAnalysisResult executeAnalysis(final DashboardAnalysis
	 * analysis) throws ComputingException, InterruptedException { final
	 * AnalysisCompute compute = new AnalysisCompute(analysis.getUniverse());
	 * return compute.executeAnalysis(analysis); }
	 */
	public void executeAnalysis(final DashboardAnalysis analysis, QueryWriter writer, boolean lazy)
			throws ComputingException, InterruptedException {
		final AnalysisCompute compute = new AnalysisCompute(analysis.getUniverse());
	...
	public List<SimpleQuery> reinject(final DashboardAnalysis analysis)
			throws ComputingException, InterruptedException {
		try {
			final AnalysisCompute compute = new AnalysisCompute(analysis.getUniverse());
	 ...
	 * @throws ExecutionException
	 */
	public Facet glitterFacet(Universe universe, Domain domain, DashboardSelection sel, Axis axis, String filter,
			int offset, int size, Integer timeoutMs) throws ComputingException, InterruptedException, TimeoutException {
		try {
			DomainFacetCompute compute = new DomainFacetCompute(universe);
