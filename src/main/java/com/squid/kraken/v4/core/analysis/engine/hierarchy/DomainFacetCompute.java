...
public class DomainFacetCompute extends FacetBuilder {
	 ...
	 * @param size
	 * @param timeoutMs
	 *            - if null the call is blocking; if timeoutMs>0 then it will
	 *            wait for the facet to be complete with timeout; if timeout==0
	 *            it won't block
	 * @return
	 ...
	 */
	public Facet computeDimensionFacets(Domain domain, DashboardSelection sel, Axis axis, String filter, int offset,
			int size, Integer timeoutMs)
					throws ComputingException, InterruptedException, ExecutionException, TimeoutException {
		//
		...
		if (index.getStatus() == Status.STALE) {
			...
			hierarchy.isDone(index, timeoutMs);// it should either return true
												// or fail
		}
		//
	...
	public Collection<Facet> computeDomainFacets(Domain domain, DashboardSelection sel)
		...
		for (DimensionIndex index : hierarchy.getDimensionIndexes()) {
			...
			if (!hierarchy.isSegment(index) // exclude segments
					&& checkHasRole(universe, index.getDimension()) // check
																	// user role
					&& !image.isInstanceOf(IDomain.OBJECT) // hide the objects
															// for now
			) {
				// T70
				if (index.isVisible()) {
	...
	private Facet computeDimensionFacets(Domain domain, DimensionIndex index, DashboardSelection sel, String filter,
			int offset, int size) {
		//
		try {
			Status status = index.getStatus();// copy the status now to avoid
												// race-conditions?
			...
			// if the ES cluster is not available, the error is not catch by the
			// index (the state may resolve by itself)
	...
	public List<DimensionMember> populateDimensionFacets(DimensionIndex index, DashboardSelection sel, String filter,
			int offset, int size) {
		...
		if (index.getFullOptions() != null && index.getFullOptions().isHidden()) {
			...
			DimensionOption option = DimensionOptionUtils.computeContextOption(index.getDimension(),
					universe.getContext());
		...
		// create a list of all the selected members in the facet parent
		// hierarchy
