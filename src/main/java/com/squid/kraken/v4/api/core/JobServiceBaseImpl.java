 ...
 *        the JobResult type
 */
public abstract class JobServiceBaseImpl<T extends ComputationJob<PK, R>, PK extends GenericPK, R extends JobResult> extends GenericServiceImpl<T, PK> {

  private static final Logger logger = LoggerFactory.getLogger(JobServiceBaseImpl.class);

  public static enum OutputFormat {
    JSON, XLS, CSV, XML, XLSX
  };
  ...
  public void initJobsGC(int temporaryJobMaxAgeInSeconds) {
    try {
      ...
      logger.info("Stopping " + findAllNotDone.size() + " 'zombie' running tasks.");
      for (T job : findAllNotDone) {
        job.setError(new ComputationJob.Error("Task stopped", "Stopped due to server restart."));
      ...
      ModelGC<T, PK> gc = new ModelGC<T, PK>(temporaryJobMaxAgeInSeconds, this, type);
      jobsGCThread = jobsGC.scheduleWithFixedDelay(gc, 0, 1, TimeUnit.HOURS);
  ...
  public void shutdownJobsExecutor() {
    try {
      ...
      logger.info("stopping jobs GC scheduler for " + this.getClass().getName());
      if (jobsGC != null) {
        jobsGC.shutdown();
   ...
   * If results are not found in the cache, and reRunIfNoResults is true, then the job computation is triggered and read the results.<br>
   *
   * @param ctx
   * @param jobId
   * @param timeout
   ...
   * @param maxResults
   * @param startIndex
   * @return a {@link JobResult} or throws an APIException if job computing is in progress or has failed.
   * @throws InterruptedException
   */
  public R readResults(AppContext ctx, PK jobId, Integer timeoutMs, boolean reRunIfNoResults, Integer maxResults, Integer startIndex, boolean lazy, Integer retryDelayMs) {
    ...
    if (job.getStatus().equals(Status.RUNNING)) {
      ...
      if (job.getTemporary()) {
        ...
        if (reRunIfNoResults) {
          ...
          results = readResults(ctx, jobId, timeoutMs, false, maxResults, startIndex, lazy, retryDelayMs);
        } else {
          throw new APIException(job.getError().getMessage(), ctx.isNoError(), ApiError.COMPUTING_FAILED);
        }
      }
   ...
   * @param an
   *        optional ResultSetWriter (default is ResultSetWriterCSV)
   * @throws InterruptedException
   */
  public void writeResults(OutputStream out, AppContext ctx, T job, Integer retryDelayMs, Integer timeoutMs, boolean reRunIfNoResults, Integer maxResults, Integer startIndex, boolean lazy, OutputFormat outFormat, OutputCompression outputCompression, ExportSourceWriter writer) throws InterruptedException {
    boolean jobRead = ((job.getId() != null) && (job.getId().getObjectId() != null));
    if ((outFormat == OutputFormat.JSON) && (outputCompression == OutputCompression.NONE || outputCompression == OutputCompression.NULL)) {
      // return the results the default way
      JobResult results;
      if (jobRead) {
        results = readResults(ctx, job.getId(), timeoutMs, reRunIfNoResults, maxResults, startIndex, lazy, retryDelayMs);
      } else {
      ...
      if (writer == null) {
        if (outFormat == OutputFormat.CSV) {
          ...
          writer = new ExportSourceWriterXLSX(settings);
  ...
  protected abstract R paginateResults(R results, Integer maxResults, Integer startIndex);
   ...
   * If a Job with the same Id already exist then (if non temporary, update it) and re-compute.<br>
   * If a Job with the same Id does not exist then create it and compute.<br>
   * If a created job has no jobId then it will be a temporary job. If a timeout is set, this method will return once the job is done or until the timeout is reached. If not, the job will simply be created but not executed.
   *
   * @param customerId
   * @param job
   * @param timeout
   *        in milliseconds
   * @param run
   *        should the job start right away
   * @return a job.
   */
  public T store(AppContext ctx, T job, Integer timeout, boolean run, Integer maxResults, Integer startIndex, boolean lazy) {
    ...
    if (existingJob != null) {
      if (!existingJob.getTemporary()) {
        ...
        jobToStart = job;
        jobctx = ServiceUtils.getInstance().getRootUserContext(ctx.getCustomerId());
      } else {
        // just recompute
  ...
  private T runJob(AppContext ctx, T jobToStart, Integer timeout, boolean forceAutoRun, Integer maxResults, Integer startIndex, boolean lazy) {
    return this.runJob(ctx, jobToStart, timeout, forceAutoRun, maxResults, startIndex, lazy, false);
  }

  /**
   * Run a job (if required). Conditions are : if its status is not RUNNING or if the job is autorun.
   *
   * @param ctx
   * @param jobToStart
   * @param timeout
   *        if not set will wait for results
   * @param jobResultsCache
   * @param forceAutoRun
   *        bypass the autorun conditions check.
   * @param lazy
   *        use to return a Job only if the analysis is already in cache; else a HTTP 204 is returned
   * @param returnJob
   *        if returnJob is set to true, an AnalysisJob is always return, event in case NotInCacheException * This is used to return a job if it is newly created or updated (e.g. this is called by store)
   * @return
   */
   ...
   * @return
   */
  private T runJob(AppContext ctx, T jobToStart, OutputStream out, boolean forceAutoRun, Integer timeout, ExportSourceWriter writer, Integer maxResults, Integer startIndex, boolean lazy) {
    if ((!jobToStart.getStatus().equals(Status.RUNNING)) || (forceAutoRun)) {
      JobTask<T, PK, R> task = new JobTask<T, PK, R>(ctx, jobToStart, computer, type, out, writer);
