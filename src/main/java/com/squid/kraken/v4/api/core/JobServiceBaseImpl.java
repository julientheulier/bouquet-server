...
public abstract class JobServiceBaseImpl<T extends ComputationJob<PK, R>, PK extends GenericPK, R extends JobResult>
	 ...
	 */
	public R readResults(AppContext ctx, PK jobId, Integer timeoutMs, boolean reRunIfNoResults, Integer maxResults,
			Integer startIndex, String lazy, Integer retryDelayMs) {
	 ...
	 * @throws InterruptedException
	 */
	public void writeResults(OutputStream out, AppContext ctx, T job, Integer retryDelayMs, Integer timeoutMs,
			boolean reRunIfNoResults, Integer maxResults, Integer startIndex, String lazy, OutputFormat outFormat,
		...
		if ((outFormat == OutputFormat.JSON)
			...
			JobResult results;
			if (jobRead) {
				results = readResults(ctx, job.getId(), timeoutMs, reRunIfNoResults, maxResults, startIndex, lazy,
						retryDelayMs);
			} else {
	...
	public T store(AppContext ctx, T job) {
		return store(ctx, job, null, true, null, null, null);
	}

	public T store(AppContext ctx, T job, Integer timeout) {
		return store(ctx, job, timeout, true, null, null, null);
	}

	public T store(AppContext ctx, T job, Integer timeout, Integer maxResults, Integer startIndex) {
		return store(ctx, job, timeout, true, maxResults, startIndex, null);
	}

	public T store(AppContext ctx, T job, Integer timeout, Integer maxResults, Integer startIndex, String lazy) {
	 ...
	 * @return a job.
	 */
	public T store(AppContext ctx, T job, Integer timeout, boolean run, Integer maxResults, Integer startIndex,
			String lazy) {
		T jobToStart = null;
	...
	private T runJob(AppContext ctx, T jobToStart, Integer timeout, boolean forceAutoRun, Integer maxResults,
	 ...
	 * @param lazy
	 *            use to return a Job only if the analysis is already in cache;
	 *            else a HTTP 204 is returned
	 * @param returnJob
	 *            if returnJob is set to true, an AnalysisJob is always return,
	 *            event in case NotInCacheException * This is used to return a
	 *            job if it is newly created or updated (e.g. this is called by
	 *            store)
	 * @return
	 */
	private T runJob(AppContext ctx, T jobToStart, Integer timeout, boolean forceAutoRun, Integer maxResults,
			Integer startIndex, String lazy, boolean returnJob) {
		if ((!jobToStart.getStatus().equals(Status.RUNNING)) && ((jobToStart.getAutoRun() == true) || forceAutoRun)) {
			JobTask<T, PK, R> task = new JobTask<T, PK, R>(ctx, jobToStart, computer, type, maxResults, startIndex,
					lazy, returnJob);
			// and start it
	 ...
	 * @return
	 */
	private T runJob(AppContext ctx, T jobToStart, OutputStream out, boolean forceAutoRun, Integer timeout,
			ExportSourceWriter writer, Integer maxResults, Integer startIndex, String lazy) {
		if ((!jobToStart.getStatus().equals(Status.RUNNING)) || (forceAutoRun)) {
			...
			if (writer == null) {
				...
				if (jobToStart.getError() != null) {
					throw new APIException(jobToStart.getError().getMessage(), ctx.isNoError(),
							ApiError.COMPUTING_FAILED);
				}
			}
		}
		return jobToStart;
