...
@Api(value = "analysisjobs", authorizations = {
...
public class AnalysisJobServiceRest extends BaseServiceRest {
	...
	public Response viewSQL(@PathParam("projectId") String projectId, @PathParam(PARAM_NAME) String jobId,
			@ApiParam(value = "if true return the SQL code as a html page and apply a prettyfier (default to true)") @QueryParam("prettyfier") Boolean prettyfierFlag)
					throws ComputingException, InterruptedException, ScopeException, SQLScopeException,
					RenderingException {
		final ProjectAnalysisJobPK id = new ProjectAnalysisJobPK(userContext.getCustomerId(), projectId, jobId);
	...
	public Response getResultsAsFile(@PathParam("projectId") String projectId, @PathParam(PARAM_NAME) String jobId,
	...
	@Path("{" + PARAM_NAME + "}" + ".{ext}")
	@ApiOperation(value = "Gets an AnalysisJobs' results as a file")
	public Response postAndGetResultsAsFile(@PathParam("projectId") String projectId,
			@PathParam(PARAM_NAME) String jobId, @PathParam("ext") String ext,
	...
	private Response returnResultsAsFile(String projectId, String jobId, String ext, Integer timeout,
		...
		if (template == null) {
			...
			final ProjectAnalysisJobPK id = new ProjectAnalysisJobPK(userContext.getCustomerId(), projectId, jobId);
			final ProjectAnalysisJob job = new ProjectAnalysisJob(id);
			return this.renderTemplate(userContext, job, timeout, maxResults, startIndex, lazy, type, template,
					setFileName);
		}
	}
	...
	public Response renderTemplate(AppContext ctx, final ProjectAnalysisJob job, final Integer timeout,
		...
		return response.type(type + "; charset=UTF-8").build();
	...
	public ProjectAnalysisJob readOrStore(@PathParam("projectId") String projectId, @PathParam(PARAM_NAME) String jobId,
			@QueryParam("analysisJob") String analysisJobJSON, @QueryParam("timeout") Integer timeout) {
		if (analysisJobJSON == null) {
			ProjectAnalysisJob read = delegate.read(userContext,
					new ProjectAnalysisJobPK(userContext.getCustomerId(), projectId, jobId));
			return read;
		} else {
	 ...
	 * If a Job with the same Id does not exist then create it and compute it.
	 * <br>
	...
	public Response store(@PathParam("projectId") String projectId, @ApiParam(required = true) ProjectAnalysisJob job,
	...
	@Path("")
	@ApiOperation(value = "Create, update and/or re-compute an AnalysisJob")
	public Response put(@PathParam("projectId") String projectId, @ApiParam(required = true) ProjectAnalysisJob job,
			@QueryParam("timeout") Integer timeout,
			@ApiParam(value = "paging size") @QueryParam("maxResults") Integer maxResults,
			@ApiParam(value = "paging start index") @QueryParam("startIndex") Integer startIndex,
			@ApiParam(value = "if true, get the analysis only if already in cache, else throw a NotInCacheException; if noError returns a null result if the analysis is not in cache ; else regular analysis", defaultValue = "false") @QueryParam("lazy") String lazy,
	...
	public Set<AccessRight> storeAccessRights(@PathParam("projectId") String projectId,
		...
		return delegate.storeAccessRights(userContext,
				new ProjectAnalysisJobPK(userContext.getCustomerId(), projectId, jobId), accessRights);
	}
