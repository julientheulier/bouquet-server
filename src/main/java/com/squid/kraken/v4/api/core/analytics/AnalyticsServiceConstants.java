...
public interface AnalyticsServiceConstants {
	...
	public static final String ROOTFILTERS_PARAM = "rootFilters";
	...
	public static final String ROOTFILTERS_DOC = "Define the root filters to apply to your bookmark (always applied when an analysis is derived from this bookmark). A filter must be a valid conditional expression.";
	...
	public static final String ORDERBY_DOC = "Define how to sort the results. You can specify a colun either by it's index (starting at zero by groupBy, then metrics), or using an expression. Use the function DESC() and ASC() to modify the sort order. The expression must be a column, or at least a hierarchical parent of a column (in that case a groupBy may be added automatically to the query). You can indicates where to place null values in the result set using (ASC|DESC)_NULLS_(FIRST|LAST) functions, for ex DESC_NULLS_LAST(expr)";
